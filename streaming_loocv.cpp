...
void update_weights() {
  ...
  if (gMode == 1) {
    ...
    // Evaluation: compute metrics only
    float loss = gLossActivation.forward(gDense2->output, gTargetTensor);
    gAccumulatedLoss += loss;
    gTestSampleCount++;
    ...
    for (int c = 0; c < kOutputClasses; c++) {
      ...
      if (gFrameProbs(0, c) > gFrameProbs(0, predClass > 0 ? predClass : 0)) {
        predClass = c;
      }
    }
...
// Additional function to report LOOCV fold results
void report_fold_results() {
  if (gMode != 2 || gTestSampleCount == 0) {
    return;
  }

  float meanLoss = gAccumulatedLoss / gTestSampleCount;
  float accuracy = static_cast<float>(gCorrectPredictions) / gTestSampleCount;
