...
const int HT_N_CLASSES = 3; // number of classes
const int HT_N_FEATURES =
    224; // number of features from signal processing (N_CHANNELS * 4)
const int HT_MAX_NODES = 64; // max nodes in a tree
const int HT_MAX_LEAVES = 20;
const int HT_GRACE_PERIOD =
    50; // how many samples to observe before trying split
const float HT_DELTA =
    0.01f; // confidence level for Hoeffding bound (1 - delta is confidence)
...
private:
  ...
  float calculateEstimatedGain(const LeafStats *s, int featureIdx,
                               float threshold, float currentGini) {
    ...
    // initialize class counts for left and right splits
    for (int i = 0; i < HT_N_CLASSES; i++) {
      leftSamples[i] = 0;
      rightSamples[i] = 0;
    }
...
public:
  ...
  uint8_t predict(const float *features) {
    ...
    for (int c = 1; c < HT_N_CLASSES; c++) {
      if (nodes[curr].stats->nSamples[c] >
          nodes[curr].stats->nSamples[bestClass]) {
        bestClass = c;
      }
    }
    return bestClass;
  ...
  void train(const float *features, uint8_t label) {
    int curr = 0;
    // find correct leaf for the sample
    while (nodes[curr].split_feature != -1) {
      if (features[nodes[curr].split_feature] <= nodes[curr].threshold) {
        curr = nodes[curr].left;
      } else {
    ...
    // check for split post grace period
    if (s->totalSamples >= HT_GRACE_PERIOD && leafCount < HT_MAX_LEAVES &&
        nodeCount < HT_MAX_NODES - 2) {
      attemptSplit(curr);
    }
  }
  ...
  void attemptSplit(int nodeIdx) {
    ...
    for (int f = 0; f < HT_N_FEATURES; f++) {
      // approximate threshold as the mean feature value across all classes
      float thresholdAttempt = 0;
      for (int c = 0; c < HT_N_CLASSES; c++)
        thresholdAttempt += s->sum[f][c];
      thresholdAttempt /= s->totalSamples;

      // calc gini gain for feature and threshold
    ...
    if (bestFeature != -1 && (bestGiniGain - secondBestGiniGain > epsilon ||
                              epsilon < HT_TIE_THRESHOLD)) {
      float finalThresh = 0;
      for (int c = 0; c < HT_N_CLASSES; c++)
        finalThresh += s->sum[bestFeature][c];
